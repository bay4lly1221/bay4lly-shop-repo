(function () {
    const pluginInfo = { "name": "GeckoMeshy", "id": "geckomeshy", "version": "1.0.4", "repository": "https://github.com/Bay4lly/geckomeshy" };

    const pluginSettings = [
        {
            id: 'geckomeshy_normalized_mesh_uvs',
        ...
        {
            id: 'geckomeshy_skip_mesh_normals',
        ...
        },
        {
            ...
            onChange: (value) => {
                ...
                if (Formats['geckolib_model']) {
                    Formats['geckolib_model'].single_texture = !value;
                }
            },
        },
    ];
    ...
    const listOfFunctions = [geckomeshyOnParseEvent.name, geckomeshyOnCompileEvent.name, geckomeshyOnBedrockCompileEvent.name];

    Plugin.register(pluginInfo.id, {
        title: pluginInfo.name, //GeckoMeshy
        author: 'Shadowkitten47, Bay4lly',
        creation_date: '2024-09-28',
        icon: 'diamond',
        description: 'Enables the use of a meshes in bedrock and GeckoLib formats and to export them to Minecraft',
        variant: 'both',
        version: pluginInfo.version, //1.0.4
        tags: ['Minecraft: Bedrock Edition', 'Entity Models', 'Mesh', 'GeckoLib'],
        has_changelog: true,
        min_version: '4.10.4',
        ...
        onload() {
            ...
            bedrock.single_texture = !settings['geckomeshy_force_textures']?.value;
            bedrock_old.single_texture = !settings['geckomeshy_force_textures']?.value;
            if (Formats['geckolib_model']) {
                Formats['geckolib_model'].single_texture = !settings['geckomeshy_force_textures']?.value;
            }
            ...
            codec.on('parsed', geckomeshyOnParseEvent);
            codec.on('compile', geckomeshyOnBedrockCompileEvent);

            codec = Codecs['bedrock_old'];
            purgeEvents(codec);

            codec.on('parsed', geckomeshyOnParseEvent);
            codec.on('compile', geckomeshyOnCompileEvent);
    ...
    function geckomeshyOnCompileEvent({ model, options }) {
    ...
    function geckomeshyOnParseEvent({ model }) {
    ...
    function geckomeshyOnBedrockCompileEvent({ model, options }) { //Extra step for non-legacy bedrock
        model = model['minecraft:geometry'][0];
        geckomeshyOnCompileEvent({ model, options });
    }
    ...
    function compileMesh(polyMesh, mesh) {
        polyMesh ??=
        {
            normalized_uvs: settings["geckomeshy_normalized_mesh_uvs"].value,
            positions: [],
            normals: [],
            uvs: [],
            polys: []
        ...
        //Spread opertator fails here we loop for each
    ...
    function uvOnSave(...uv) {
        uv[1] = Project.texture_height - uv[1] //Invert y axis
        if (!settings["geckomeshy_normalized_mesh_uvs"].value) return uv
    ...
    function getVertexNormal(mesh, vertexKey, vertexFacesMap) {
        if (settings["geckomeshy_skip_mesh_normals"].value) return [0, 1, 0];
        let normalSum = [0, 0, 0];
        let faceCount = 0;
