...
(function() {
    ...
    const REPO_URL = 'https://raw.githubusercontent.com/Bay4lly/Shop-Repo/main/';
    ...
    const Bay4llyShop = {
        ...
        async installPlugin(pluginId) {
            ...
            try {
                const resp = await fetch(`${REPO_URL}plugins/${plugin.folder}/plugin.js`);
                if (!resp.ok) throw new Error('Failed to download plugin');
                
                const code = await resp.text();

                if (Blockbench.isDesktop) {
